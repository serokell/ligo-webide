...
export default class LocalProjectManager extends BaseProjectManager {
  static async createProject(options, stage = '') {
    ...
    const data = {
      ...
      projectRoot: '.workspaces/' + options.name,
      __v: 0
    }
  ...
  static async openProject(obj, name) {
    ...
    const setF = async (path, content, cb) => {
      ...
      await fileOps.current.fs.ensureDir(path.substr(0, path.lastIndexOf('/')))
      try {
        await window.remixFileSystem.writeFile(path, content, 'utf8')
      } catch (e) {
        return false
    ...
    if (await workspaceExists(data.name)) {
      ...
      await fileOps.current.fs.ensureDir('.workspaces/' + data.name)
    }

    for (const key of Object.keys(obj)) {
      try {
        await setF('.workspaces/' + data.name + '/' + key, obj[key].content)
      } catch (error) {
        console.error(error)
      }
    }
  ...
  async readFile(filePath, cb) {
    return await fileOps.current.readFile(filePath, cb)
  }
  ...
  async copyFolderToJson (directory, visitFile, visitFolder) {
    ...
    let json = await this._copyFolderToJsonInternal(directory, ({ path, content }) => {
  ...
  async deleteFile(node) {
    ...
    if (response === 0) {
      if (node.children) {
        await fileOps.current.deleteFolder(node.path)
      } else {
